    ...
    function get_warranty_data($arrBookings, $is_excel = false){
        ...
        foreach ($arrBookings as $booking_id => $rec_data) {
            ...
            if(!empty($rec_data['service_id']))
            {
                ...
                $arrOrWhere["((warranty_plans.service_id = '".$rec_data['service_id']."' AND appliance_model_details.id IS NULL) and date(warranty_plans.period_start) <= '".$purchase_date."' and date(warranty_plans.period_end) >= '".$purchase_date."' and warranty_plans.partner_id = '".$rec_data['partner_id']."' AND (plan_depends_on = 2))"] = null; 
            }
            else
            {
                //removes the single as well as double quotes from start and end
