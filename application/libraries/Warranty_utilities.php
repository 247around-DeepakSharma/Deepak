...
class Warranty_utilities {
    ...
    function get_warranty_data($arrBookings, $is_excel = false){
        ...
        foreach ($arrBookings as $booking_id => $rec_data) {
            ...
            if(!empty($rec_data['service_id']))
            {
                ...
                $arrOrWhere["((warranty_plans.service_id = '".$rec_data['service_id']."' AND appliance_model_details.id IS NULL) and date(warranty_plans.period_start) <= '".$purchase_date."' and date(warranty_plans.period_end) >= '".$purchase_date."' and warranty_plans.partner_id = '".$rec_data['partner_id']."' AND (plan_depends_on = ". WARRANTY_PLAN_ON_PRODUCT ."))"] = null; 
            }
            else
            {
                //removes the single as well as double quotes from start and end
    ...
    function match_warranty_status_with_request_type($arrBookings, $arrBookingsWarrantyStatus){
        ...
        $arr_warranty_status = ['IW' => ['In Warranty', 'Presale Repair', 'AMC', 'Repeat', 'Installation', 'PDI', 'Demo', 'Tech Visit', 'Replacement', 'Free Remote Assistance'], 'OW' => ['Out Of Warranty', 'Out Warranty', 'AMC', 'Repeat', 'PDI', 'Tech Visit', 'Free Remote Assistance'], 'EW' => ['Extended', 'AMC', 'Repeat', 'PDI', 'Tech Visit', 'Free Remote Assistance']];
