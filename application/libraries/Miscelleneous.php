            ...
            case UPCOUNTRY_LIMIT_EXCEED:
                ...
                if(!empty($is_upcountry)){
                    if($data['message'] !== UPCOUNTRY_LIMIT_EXCEED){
                        ...
                        $this->My_CI->booking_model->update_booking($booking_id, $booking);
                        $this->process_cancel_form($booking_id, "Pending", UPCOUNTRY_CHARGES_NOT_APPROVED, "", $agent_id, $agent_name);
               ...
               if(!empty($unit_details)){
                   ...
                   if(!empty($sc_data1)){
                       foreach ($sc_data1 as $value1) {
                           ...
                           if(empty($unit_details)){
                               log_message('info', __FUNCTION__ . " Booking Unit details not exist  unit_id" . print_r(value['unit_details_id'], true));
