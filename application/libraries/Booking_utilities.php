...
function get_qr_code_response($booking_id, $amount_due, $pocNumber, $user_id, $userPhone, $services, $partner_id){
   ...
   /*
    ...
    function get_installation_date_of_booking($arrBooking){
        ...
        if(!empty($arrBooking[0]['partner_id']) && !empty($arrBooking[0]['service_id']) && !empty($arrBooking[0]['brand']) && !empty($arrBooking[0]['model_number']) && !empty($arrBooking[0]['serial_number']) && !empty($arrBooking[0]['booking_id'])){
            $partner_id = $arrBooking[0]['partner_id'];
            $service_id = $arrBooking[0]['service_id'];
            $brand = $arrBooking[0]['brand'];
            $model_number = $arrBooking[0]['model_number'];
            ...
            if($check_warranty_from == WARRANTY_ON_DOI){
                ...
                $booking_data = $this->My_CI->booking_model->get_data_for_duplicate_serial_number_check($serial_number, $booking_id, true, $partner_id, $service_id, $brand, $model_number);
