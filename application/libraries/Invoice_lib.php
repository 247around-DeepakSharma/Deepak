...
//                 array_push($int_invoice, $explode[1] + 1);
        ...
        if ($show_courier_servicable_area) {
            if (!empty($spare_details[0][0]['courier_name'])) {
                ...
                if (!empty($serviceable_area)) {
                    ...
                    $excel_data['excel_data']['courier_servicable_area'] = $couriers_name;
                }
            }
        }
                ...
                if(!empty($spare_id)){
                    ...
                    if(!empty($sp)){
                        ...
                        array('hsn_code_details.id' => $sp[0]['shipped_hsn_code']));
                            ...
                            $tmp_arr['hsn_code'] = $hsn_code_arr[0]['hsn_code'];

                        }

                         if(!empty($sp[0]['shipped_inventory_id'])){
    ...
    /**
     * @desc This function is used to return challan value for the inventory
            ...
            foreach ($spare_parts_details as $spare_key => $spare_parts_details_value) {
                if (!empty($spare_parts_details_value[0]['shipped_inventory_id']) && !empty($spare_parts_details_value[0]['parts_shipped'])) {
                    ...
                    if (!empty($inventory_master_data)) {
                        $spare_parts_details[0][$spare_key]['part_number'] = $inventory_master_data[0]['part_number'];
                        $spare_parts_details[0][$spare_key]['inventory_id'] = $spare_parts_details_value[0]['requested_inventory_id'];
                    } else {
                        $spare_parts_details[0][$spare_key]['part_number'] = '';
                    }
                }
    ...
    /**
     * @desc Generate Challan To partner
     * @param type $booking_id
     * @return boolean
     */
            ...
            foreach ($spare_parts_details as $spare_key => $spare_parts_details_value) {
                if (!empty($spare_parts_details_value[0]['shipped_inventory_id']) && !empty($spare_parts_details_value[0]['parts_shipped'])) {
                    ...
                    if (!empty($shipped_inventory_id)) {
                        $whereinventory = array('inventory_id' => $shipped_inventory_id);
                        $inventory_master_data = $this->ci->inventory_model->get_inventory_master_list_data('part_number', $whereinventory);
                        $spare_parts_details[$spare_key][0]['part_number'] = $inventory_master_data[0]['part_number'];
                        $spare_parts_details[$spare_key][0]['inventory_id'] = $shipped_inventory_id;
                    } else {
...
//            }
    ...
    function settle_inventory_invoice_annexure($postData, $from_gst_id = "", $to_gst_id = "") {
        ...
        foreach ($postData as $value) {
            if (!empty($value['inventory_id'])) {
                ...
                $this->invoices_not_found($value);
                array_push($not_updated, (isset($value['booking_id'])?$value['booking_id']:''));
                log_message('info', _METHOD_ . " Inventory ID Missing. Spare Invoice is not generating for booking id " . (isset($value['booking_id'])?$value['booking_id']:'') . " Inventory id " . $value['inventory_id']);
            }
        }
    ...
    function settle_inventory_challan_annexure($processPostData, $value, $to_gst_id, $from_gst_id) {
        $where = array('inventory_id' => $value['inventory_id'],
            'sender_entity_id' => $value['booking_partner_id'],
            "challan_items_details.is_settled" => 0,
            "challan_details.type" => 1);
        if (!empty($to_gst_id)) {
            $where['to_gst_number_ids'] = $to_gst_id;
        }
        if (!empty($from_gst_id)) {
            $where['from_gst_number_id'] = $from_gst_id;
        }
        $order_by = array('column_name' => "challan_items_details.id", 'param' => 'asc');

        $unsettle = $this->ci->inventory_model->get_challan_with_item_details('challan_items_details.*, '
                . 'challan_details.challan_id as invoice_id, from_gst_number, to_gst_number',
                $where, $order_by);
        ...
        if (!empty($unsettle)) {
            ...
            foreach ($unsettle as $key => $b) {
                ...
                $restQty = $b['quantity'] - $b['settled_quantity'];
                if ($restQty == $qty) {
                    ...
                    if (!empty($s)) {
                        ...
                        if (!array_key_exists($s['from_state_code'] . "-" . $s['to_state_code'], $processPostData[1])) {
                            ...
                            $processPostData[1][$s['from_state_code'] . "-" . $s['to_state_code']]['from_state_code'][0] = $s['from_state_code'];

                            $processPostData[1][$s['from_state_code'] . "-" . $s['to_state_code']]['mapping'][0] = $mapping;
                        } else {

                            array_push($processPostData[1][$s['from_state_code'] . "-" . $s['to_state_code']]['data'], $s);
                            array_push($processPostData[1][$s['from_state_code'] . "-" . $s['to_state_code']]['mapping'], $mapping);
                        }
                    ...
                    $s = $this->get_array_settle_data($b, $inventory_details, $qty, $value);
                    if (!empty($s)) {
                        $this->ci->invoices_model->update_invoice_breakup(array('id' => $b['id']), array('is_settled' => 0, 'settled_quantity' => $b['is_settled'] + $qty));
                        $mapping = array('incoming_invoice_id' => $b['invoice_id'], 'settle_qty' => $qty, 'create_date' => date('Y-m-d H:i:s'),
                            "inventory_id" => $value['inventory_id'], 'spare_id' => $spare_id, 'rate' => $s['rate']);
