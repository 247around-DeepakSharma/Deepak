...
class accounting_model extends CI_Model {
    ...
    function get_accounting_s_p_report($from_date, $to_date, $partner_vendor, $is_challan_data, $invoice_data_by, $payment_type) {
        ...
        $sql = "SELECT invoice_details.invoice_id,invoice_details.hsn_code,product_or_services, vendor_partner, e1.gst_number as to_gst_number, "
                ...
                . " LEFT JOIN partners ON vendor_partner = 'partner' AND partners.id = vpi.vendor_partner_id "
                . " JOIN invoice_details ON invoice_details.invoice_id = vpi.invoice_id "
                . " LEFT JOIN entity_gst_details as e1 ON e1.id = invoice_details.to_gst_number "
                . " LEFT JOIN entity_gst_details as e2 ON e2.id = invoice_details.from_gst_number "
                . " WHERE "
                . " type_code = '$payment_type' AND vpi.type NOT IN ('".BUYBACK_VOUCHER."', '".BUYBACK_TYPE."') "
                . " AND  vendor_partner = '$partner_vendor'"
                . " AND sub_category != '"._247AROUND_CANCELLED."' "
                . " AND vertical != '"._247AROUND_CANCELLED."' $where "
                . " GROUP By invoice_details.invoice_id, "
                . " invoice_details.product_or_services, "
                . " invoice_details.`igst_tax_rate`, "
                . " invoice_details.`sgst_tax_rate`, "
                . " invoice_details.`cgst_tax_rate`, invoice_details.hsn_code";
