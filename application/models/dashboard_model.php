...
//        }
    ...
    function get_cancelled_booking_graph_data($startDate, $endDate){
        $sql = "SELECT employee.full_name, employee.id, SUM(IF (new_state = 'Cancelled_Rejected', 1, 0)) as completed_rejected,
                    ...
                    SUM(IF ((old_state = 'InProcess_Cancelled' AND (new_state = 'Completed' OR new_state = 'Completed_With_Rating')), 1, 0)) as edit_completed,
                    (SUM(IF (new_state = 'Cancelled', 1, 0)) - SUM(IF (new_state = 'Cancelled_Approved', 1, 0))) as edit_cancelled,
                    (SUM(IF (new_state = 'Cancelled_Rejected', 1, 0)) + SUM(IF (new_state = 'Cancelled_Approved', 1, 0)) + SUM(IF ((old_state = 'InProcess_Cancelled' AND (new_state = 'Completed' OR new_state = 'Completed_With_Rating')), 1, 0)) + (SUM(IF (new_state = 'Cancelled', 1, 0)) - SUM(IF (new_state = 'Cancelled_Approved', 1, 0)))) as total_bookings
