...
//            $min_threshold_distance = UPCOUNTRY_MIN_DISTANCE;
    ...
    function upcountry_partner_invoice($partner_id, $from_date, $to_date, $spare_requested_unit_id = ""){
        $sql = "SELECT CASE WHEN (bd.partner_id = '".PAYTM_ID."' ) "
                ...
                . " COUNT(DISTINCT(bd.booking_id)) AS count_booking, partner_upcountry_rate, upcountry_pincode, services, bd.district as city, bd.state, pdm.district as upcountry_city, pdm.state as upcountry_state, booking_pincode, sub_vendor_id"
                . " FROM `booking_details` AS bd LEFT JOIN `pincode_district_mapping` AS pdm ON bd.upcountry_pincode=pdm.pincode, booking_unit_details AS ud, services "
                . " WHERE ud.booking_id = bd.booking_id "
...
//    }
    ...
    function getpincode_upcountry_local($return_array = false,$where=''){
        $sql = "SELECT distinct pincode as 'Pincode',services as 'Appliance', district as 'District', case when is_upcountry =1 THEN ('Upcountry') ELSE 'Local' END as 'Upcountry/Local', upcountry_pincode_services_sf_level.distance as Distance FROM `upcountry_pincode_services_sf_level`, services WHERE services.id = service_id ".$where." order by services asc ";
