...
class Database_testing_model extends CI_Model {
	...
	$sql = "SELECT `booking_id`, `partner_id`, `service_id`, `appliance_id` FROM (`booking_unit_details`) WHERE (`booking_id` = '0' OR `partner_id` = '0' OR `appliance_id` = '0') AND `create_date` >= '2016-09-01 00:00:00'";
...
//	$this->db->or_where('price_tags', NULL);
	...
	$this->db->where('booking_details.create_date >=', '2016-09-01 00:00:00');
	$query = $this->db->get();
...
//    }
	...
	$sql = "SELECT  `booking_details`.`booking_id` "
	    ...
	    . " AND  `booking_details`.`create_date` >=  '2016-09-01 00:00:00'";
	...
	$sql = "SELECT `booking_id`, `unit_details_id`, `service_center_id`, `current_status`, `internal_status`, `create_date` FROM (`service_center_booking_action`) WHERE (`booking_id` = '0' OR `unit_details_id` IS NULL OR `unit_details_id` = '0' OR `service_center_id` = '0' OR `current_status` = '0' OR `internal_status` = '0') AND `create_date` >= '2016-09-01 00:00:00'";
	$query = $this->db->query($sql);
	log_message('info', $this->db->last_query());
	//echo $this->db->last_query() . PHP_EOL;
	...
	$sql = "SELECT `booking_id`, `assigned_vendor_id`, current_status, internal_status
				...
				AND BD.booking_id NOT IN (select booking_id from service_center_booking_action) AND BD.create_date >= '2016-09-01 00:00:00'";
	$query = $this->db->query($sql);
	log_message('info', $this->db->last_query());
	if ($query->num_rows > 0) {
	    return $query->result_array();
