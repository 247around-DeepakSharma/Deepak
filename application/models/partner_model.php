 ...
 */
    ...
    function get_partner_leads_csv_for_summary_email($partner_id)
    {
        return $this->db->query(" SELECT DISTINCT '' as 'Unique id' , order_id AS 'Sub Order ID', BD.create_date AS 'Referred Date and Time',
                                        ...
                                        booking_pincode AS 'Pincode', BD.city As 'City', booking_primary_contact_no AS Phone,
                                        user_email As 'Email ID', request_type AS 'Call Type (Installation /Table Top Installation/Demo/ Service)',
                                        partner_current_status AS 'Status By Brand', '' AS 'Remarks by Brand','Service sent to vendor' AS 'Status by Snapdeal',
                                        booking_date As 'Scheduled Appointment Date(DD/MM/YYYY)', booking_timeslot AS 'Scheduled Appointment Time(HH:MM:SS)',
                                        partner_internal_status AS 'Final Status'
                                        FROM `booking_details` AS BD , booking_unit_details AS ud, services, users
                                        WHERE BD.booking_id = ud.booking_id AND BD.service_id = services.id AND BD.user_id = users.user_id
