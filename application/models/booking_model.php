...
class Booking_model extends CI_Model {
     ...
     */
    function get_booking_for_review($booking_id,$status,$whereIN,$is_partner,$offset = NULL, $perPage = NULL,$having_arr=array(),$where_arr=array()) {
        $charges = $this->service_centers_model->getcharges_filled_by_service_center($booking_id,$status,$whereIN,$is_partner,$offset,$perPage,$having_arr,$where_arr);
...
//        $this->db->join('service_center_booking_action', 'booking_details.booking_id  = service_center_booking_action.booking_id', 'left');
    ...
    function _get_queries($post,$pincode_status, $query_status,$select = "") {
        ...
        $this->db->where("(DATEDIFF(CURRENT_TIMESTAMP , STR_TO_DATE(booking_details.booking_date, '%Y-%m-%d')) >= 0 OR booking_details.booking_date = '' OR booking_details.booking_date = '0000-00-00')",NULL);
        if (!empty($post['where'])) {
            $this->db->where($post['where']);
        }
        if (!empty($post['join'])) {
