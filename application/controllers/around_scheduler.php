        ...
        foreach ($invoices as $key1 => $vendor_value) {
           ...
           $invoices_detail = $this->invoices_model->get_invoices_details(array('invoice_id in ('.$invoices_id.')'=>NULL), $invoice_select);
            ...
            foreach ($invoices_detail as $key2 => $value) {
               ...
               $this->table->add_row($value['invoice_id'], date("d/m/Y", strtotime($value['invoice_date'])), $taxable_value, $value['cgst_tax_amount'], $value['sgst_tax_amount'], $value['igst_tax_amount'], $gst_amt, $total_amount_collected);
            }
