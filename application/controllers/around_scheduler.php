 ...
 /** This function is used to get gst detail of all vendor from the taxPro API **/
    ...
    function send_notifiction_to_review_bookings() {
        ...
        foreach ($partnerArray as $partner) {
            ...
            $bookings = array_keys($tempData);
            $template = $this->booking_model->get_booking_email_template("notify_partner_to_review_bookings");
            $subject = $template[4];
            $message = vsprintf($template[0], array($partnerArray[0]['review_time_limit'], implode(",",$bookings)));
            $to =  $partnerArray[0]['primary_contact_email'];
            $bcc = $template[5];
            $cc = $template[1];
            $from = $template[2];
            echo $this->notify->sendEmail($from, $to, $cc, $bcc, $subject, $message, "", "notify_partner_to_review_bookings");
