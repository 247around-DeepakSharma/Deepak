...
class ApiDataRequest extends CI_Controller {
    ...
    */
    function spareOowEstRequestedData(){
      ...
      $this->requestData['search']['value'] = preg_replace("/[^a-zA-Z0-9]+/", "", $this->requestData['search']['value']);
        $this->requestData['column_search'] = array('booking_details.booking_id','spare_parts_details.model_number','spare_parts_details.parts_requested','spare_parts_details.serial_number');
    ...
    function customer_invoice_table_view($invoice, $no){
        ...
        $row[] = date("d-M-Y", strtotime($invoice->invoice_date));
