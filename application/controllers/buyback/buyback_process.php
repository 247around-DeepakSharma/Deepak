...
class Buyback_process extends CI_Controller {
    ...
    function process_cancelled_not_claim(){
        ...
        $post['where_in'] = array(
            ...
            'bb_order_details.internal_status' => array(TO_BE_CLAIMED));
        $post['column_order'] = array( NULL, NULL,'services', 'city','order_date', 'current_status');
        $post['column_search'] = array('bb_unit_details.partner_order_id','services', 'city','order_date','current_status');
        $post['where'] = array();
        $list = $this->bb_model->get_bb_order_list($post);
    ...
    function process_cancelled_claim_submitted(){
        ...
        $post['where_in'] = array(
            'bb_order_details.current_status' => array('Cancelled', 'Rejected'), 
            'bb_order_details.internal_status' => array(CLAIM_SUBMITTED));
        $post['column_order'] = array( NULL, NULL,'services', 'city','order_date', 'current_status');
        $post['column_search'] = array('bb_unit_details.partner_order_id','services', 'city','order_date','current_status');
        $post['where'] = array();
        $list = $this->bb_model->get_bb_order_list($post);
    ...
    function process_cancelled_claim_settled(){
        ...
        $post['where_in'] = array(
            'bb_order_details.current_status' => array('Cancelled', 'Rejected'), 
            'bb_order_details.internal_status' => array(CLAIM_SETTLED_BY_AMAZON));
        $post['column_order'] = array( NULL, NULL,'services', 'city','order_date', 'current_status');
        $post['column_search'] = array('bb_unit_details.partner_order_id','services', 'city','order_date','current_status');
        $post['where'] = array();
        $list = $this->bb_model->get_bb_order_list($post);
    ...
    function get_vendor_rejected_data(){
        ...
        $post['where_in'] = array('bb_order_details.current_status' => array(_247AROUND_BB_DELIVERED,_247AROUND_BB_TO_BE_CLAIMED),
            'bb_order_details.internal_status' => array(_247AROUND_BB_ORDER_NOT_RECEIVED_INTERNAL_STATUS,_247AROUND_BB_REPORT_ISSUE_INTERNAL_STATUS,_247AROUND_BB_NOT_DELIVERED,_247AROUND_BB_ORDER_MISMATCH));
        $post['column_order'] = array( NULL, NULL,'services', 'city','order_date', 'current_status');
        $post['where'] = array();
        $post['column_search'] = array('bb_unit_details.partner_order_id','services', 'city','order_date','current_status');
        $list = $this->bb_model->get_bb_order_list($post);
...
//        $this->push_notification_lib->create_and_send_push_notiifcation(ORDER_REJECT_INFORM_TO_CP, $receiverArray, $notificationTextArray);
    ...
    function download_order_snapshot() {
        ...
        $select = "bb_order_details.partner_order_id, service_centres.name,bb_order_details.partner_tracking_id as tracking_id,bb_order_details.acknowledge_date,services, bb_unit_details.category,city, order_date, delivery_date, bb_order_details.current_status, "
                . "bb_order_details.internal_status, bb_cp_order_action.current_status AS cp_current_status, bb_cp_order_action.internal_status AS cp_internal_status, bb_cp_order_action.remarks, partner_basic_charge, cp_basic_charge,cp_tax_charge,gst_amount,partner_sweetner_charges,bb_unit_details.cp_claimed_price, bb_unit_details.partner_invoice_id, bb_unit_details.cp_invoice_id, bb_unit_details.remarks, bb_order_details.create_date";
