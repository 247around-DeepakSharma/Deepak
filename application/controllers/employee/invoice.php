    ...
    function generate_oow_parts_invoice($spare_id) {
        ...
        if (!empty($sp_data) && empty($sp_data[0]->sell_invoice_id) && ($sp_data[0]->purchase_price > 0)) {
            ...
            if (!empty($unit_data) && empty($unit_data[0]['vendor_cash_invoice_id'])) {
                ...
                $data[0]['rate'] = sprintf("%.2f", ($data[0]['taxable_value'] / $shipped_quantity));
...
//                $to = $email_template[3];
    ...
    function generate_reverse_sale_invoice($invoice_details, $spare_data, $sub_category){
        ...
        if (!empty($vendor_details[0]['gst_no']) 
            ...
            //We are generating Credit note thats why we assigned true
            $data[0]['gst_number'] = true;
        ...
        $a = $this->_reverse_sale_invoice($invoice_id, $data, $sd, $ed, $invoice_date, $array, $sub_category, $invoice_type, $email_array);
        if($a){
             return $invoice_id;
        } else {
                ...
                if ($spare[0]['is_micro_wh'] == 1 && ($spare[0]['partner_id'] == $spare[0]['service_center_id'])) {
                    if (!empty($spare[0]['shipped_inventory_id'])) {
                        ...
                        if (!empty($unsettle['processData'])) {
                            ...
                            foreach ($unsettle['processData'] as $c_type => $invoiceTmp) {
                                if (!empty($invoiceTmp)) {
                                    foreach ($invoiceTmp as $key => $invoiceValue) {
                                        $data = array();

                                        foreach ($invoiceValue['data'] as $value) {
                                            $data[0]['description'] = ucwords($value['part_name']) . " (" . $spare[0]['booking_id'] . ") ". "  Reference Invoice ID " . $value['incoming_invoice_id'];
                                            ...
                                            if (!empty($spare[0]['gst_number']) && !empty($spare[0]['gst_status']) && !($spare[0]['gst_status'] == _247AROUND_CANCELLED || $spare[0]['gst_status'] == GST_STATUS_SUSPENDED)) {
                                                $invoice_type = "Tax Invoice";
                ...
                if ($spare[0]['is_micro_wh'] == 1 && empty($spare[0]['reverse_purchase_invoice_id'])) {
                    if (!empty($spare[0]['shipped_inventory_id'])) {
                        ...
                        if (!empty($unsettles['processData'])) {
                            ...
                            foreach ($unsettles['processData'] as $c_type => $invoiceTmp) {
                                if (!empty($invoiceTmp)) {
                                    foreach ($invoiceTmp as $key => $invoicevalue) {
                                        $tmp_k = explode('-', $key);
