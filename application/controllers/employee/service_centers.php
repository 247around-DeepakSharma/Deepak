...
//        }
    ...
    function upload_insert_upload_serial_no($upload_serial_number_pic, $unit, $partner_id, $serial_number) {
        ...
        if (!empty($upload_serial_number_pic['tmp_name'][$unit])) {
            ...
            $pic_name = $this->upload_serial_no_image_to_s3($upload_serial_number_pic, "serial_number_pic_" . $this->input->post('booking_id') . "_", $unit, "engineer-uploads", "serial_number_pic");
            if ($pic_name) {
        ...
        if (!empty($booking_id) || $booking_id != 0) {
            ...
            if (!empty($data['bookinghistory'][0])) {
                ...
                foreach ($unit_details as $value) {
                    ...
                    } else if (stristr($value['price_tags'], "Repair") || stristr($value['price_tags'], "Repeat") || stristr($value['price_tags'], EXTENDED_WARRANTY_TAG) || stristr($value['price_tags'], PRESALE_REPAIR_TAG) || stristr($value['price_tags'], GAS_RECHARGE_IN_WARRANTY) || stristr($value['price_tags'], GAS_RECHARGE_OUT_OF_WARRANTY)) {
        ...
        if (!empty($booking_id)) {
            ...
            if (!empty($sp_data)) {
                ...
                foreach ($sp_data as $key => $value){
                    ...
                    if ($value->entity_type == _247AROUND_PARTNER_STRING) {
//                        
...
//                        $inventory_stock = $this->inventory_model->get_inventory_stock_count_details($select, $where);  
                    ...
                    if (!empty($is_warehouse)) {
                        ...
                        if (!empty($warehouse_details['stock']) && $warehouse_details['stock'] > 0) {
                            if ($warehouse_details['is_micro_wh'] == 1) {
                                $actor = "vendor";
                            ...
                            } else if ($warehouse_details['is_micro_wh']== 2) {
                                $actor = "warehouse";
    ...
    function validate_serial_number_pic_upload_file() {
        ...
        if (!empty($_FILES['serial_number_pic']['tmp_name'])) {
            $allowedExts = array("png", "jpg", "jpeg", "JPG", "JPEG", "PNG", "PDF", "pdf");
            $booking_id = $this->input->post("booking_id");
            $defective_courier_receipt = $this->miscelleneous->upload_file_to_s3($_FILES["serial_number_pic"], "serial_number_pic", $allowedExts, $booking_id, "misc-images", "serial_number_pic");
            if ($defective_courier_receipt) {
