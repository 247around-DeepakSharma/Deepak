...
//            $this->session->set_userdata(['error_msg' => 'Order Id is not Valid / Filled against this Booking.']);
    ...
    function get_complete_booking_form($booking_id) {
        ...
        $data['spare_parts_details'] = $this->partner_model->get_spare_parts_by_any('spare_parts_details.*, inventory_master_list.part_number', ['booking_id' => $booking_id, 'spare_parts_details.status != "'._247AROUND_CANCELLED.'"' => NULL, 'parts_shipped is not null' => NULL, 'consumed_part_status_id is not null' => NULL], FALSE, FALSE, FALSE, ['is_inventory' => true]);        
    ...
    private function get_bookings_data_by_status($booking_status,$sfIDArray) { 
        ...
        $new_post = $this->get_filterd_post_data($post,$booking_status,'booking');
         if($this->input->post('bulk_booking_id')){
            ...
            service_centres.primary_contact_phone_1,DATE_FORMAT(STR_TO_DATE(booking_details.booking_date,'%d-%m-%Y'),'%d-%b-%Y') as booking_day,booking_details.create_date,booking_details.partner_internal_status,
    ...
    function get_service_id_by_partner(){
     ...
     function download_pending_bookings($status) {
        ...
        if($booking_status == 'Pending'){
            ...
            $select = "booking_details.booking_id,DATEDIFF(CURDATE(),STR_TO_DATE(booking_details.initial_booking_date,'%d-%m-%Y')) as Ageing,users.name as  Customer_Name,
