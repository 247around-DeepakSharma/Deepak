...
class Spare_parts extends CI_Controller {
    ...
    function get_part_delivered_to_sf($post) {
        ...
        if ($this->input->post("status") == SPARE_DELIVERED_TO_SF) {
            $post['column_order'] = array(NULL, 'spare_parts_details.booking_id', NULL, NULL, NULL, 'service_centres.name', NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, 'age_of_delivered_to_sf', NULL, NULL, NULL);
        } else {
    ...
    function defective_part_shipped_by_sf_table_data($spare_list, $no) {
        ...
        $row[] = '<span style="color:blue; pointer:cursor" class="update_awb_by_sf_from_courier_autid" data-awb-number ="'.$spare_list->awb_by_sf.'">'.$spare_list->awb_by_sf.'</span>';
        ...
        if ($this->session->userdata('user_group') == 'admin' || $this->session->userdata('user_group') == 'inventory_manager' || $this->session->userdata('user_group') == 'developer') {
            if ($request_type == SPARE_PARTS_REQUESTED || $request_type == SPARE_PART_ON_APPROVAL || $request_type == _247AROUND_CANCELLED) {
                if ($spare_list->part_requested_on_approval == '0' && $spare_list->status == SPARE_PART_ON_APPROVAL) {
                    ...
                    $row[] = '<button type="button" data-keys="' . $spare_list->part_warranty_status . '" data-booking_id="' . $spare_list->booking_id . '" data-url="' . base_url() . 'employee/spare_parts/spare_part_on_approval/' . $spare_list->id . '/' . $spare_list->booking_id . '" data-request_type="' . $spare_list->request_type . '" data-part_warranty_id="'.$spare_list->part_warranty_id.'" class="btn' . $cl . ' open-adminremarks" data-toggle="modal" id="approval_' . $no . '" data-target="#myModal2">' . $appvl_text . '</button>';
                } else {
        ...
        if (!empty($spare_parts_id)) {
            ...
            if (!empty($spare_parts_list)) {
                ...
                if (!empty($is_warehouse)) {
                    ...
                    $warehouse_details = $this->get_warehouse_details(array('inventory_id' => $inventory_id, 'state' => $sf_state[0]['state'], 
                        'service_center_id' => $spare_parts_list[0]['service_center_id'], 'model_number' => $spare_parts_list[0]['model_number'],
                        'quantity' => $req_quantity), $partner_id);
    ...
    function send_out_of_stock_mail($parts_stock_not_found, $partner_id, $data, $booking_id) {
        if (!empty($parts_stock_not_found) && !empty($booking_id)) {
            ...
            if (!empty($email_template)) {
                ...
                $get_partner_details = $this->partner_model->getpartner_data("partners.public_name,group_concat(distinct agent_filters.agent_id) as account_manager_id,primary_contact_email,owner_email", array('partners.id' => $partner_id, 'agent_filters.state' => $booking_state[0]['state']), "", 0, 1, 1, "partners.id");
            ...
            if (!empty($spare_parts_details)) {
                ...
                if ($entity_type == _247AROUND_PARTNER_STRING && $part_warranty_status == SPARE_PART_IN_WARRANTY_STATUS) {
                    ...
                    if (!empty($is_warehouse)) {
                        ...
                        $warehouse_details = $this->get_warehouse_details(array('inventory_id' => $spare_parts_details[0]['requested_inventory_id'], 'state' => $sf_state[0]['state'], 
                            'service_center_id' => $service_center_id, 'model_number' => $data['model_number'], 'quantity' => $data['quantity']), $partner_id);
    ...
    function send_email_on_change_part_warranty_status($part_warranty_status, $service_center_id, $data, $email_template_tag) {
        ...
        if (!empty($part_warranty_status)) {
            ...
            if (!empty($email_template)) {
                ...
                $get_partner_details = $this->partner_model->getpartner_data("partners.public_name,group_concat(distinct agent_filters.agent_id) as account_manager_id,primary_contact_email,owner_email", array('partners.id' => $data['partner_id'], 'agent_filters.state' => $booking_state[0]['state']), "", 0, 1, 1, "partners.id");
...
//        }
    ...
    function update_spare_parts_on_approval($code) {
        ...
        $select = 'spare_parts_details.id,spare_parts_details.defect_pic,spare_parts_details.spare_request_symptom,spare_parts_details.partner_id,spare_parts_details.shipped_quantity,spare_parts_details.quantity, spare_parts_details.date_of_request,spare_parts_details.entity_type,spare_parts_details.booking_id,spare_parts_details.date_of_purchase,spare_parts_details.model_number,'
                ...
                . 'booking_details.service_id,booking_details.partner_id as booking_partner_id,booking_details.assigned_vendor_id, spare_parts_details.parts_shipped,booking_details.user_id,booking_details.request_type,booking_details.create_date as booking_create_date,booking_details.applied_warranty_plan_id';
...
//        }
    ...
    function process_update_awb_number_sf() {
        ...
        $pre_awb_by_sf = $this->input->post("awb");
        $awb_number = $this->input->post("change_awb_number_by_sf");
        $agent_id = $this->session->userdata("id");
    ...
    function msl_summary_report($spare_id = '') {
        ...
        foreach ($list as $key => $value) {
            ...
            $select = "inventory_master_list.inventory_id,inventory_master_list.part_name,inventory_master_list.entity_id,inventory_master_list.part_number,inventory_stocks.stock,services.services,inventory_stocks.entity_id as receiver_entity_id,inventory_stocks.entity_type as receiver_entity_type";
