...
class CILogViewer {
    ...
    private function init() {
        ...
        $this->logFolderPath =  !empty($this->CI->config->item(self::LOG_FOLDER_PATH_CONFIG_KEY)) ? rtrim($this->CI->config->item(self::LOG_FOLDER_PATH_CONFIG_KEY), "/") : rtrim(APPPATH, "/") . "/logs";
        $this->logFilePattern = !empty($this->CI->config->item(self::LOG_FILE_PATTERN_CONFIG_KEY)) ? $this->CI->config->item(self::LOG_FILE_PATTERN_CONFIG_KEY) : "log-*.php";
        ...
        $this->LOG_VIEW_FILE_FOLDER = APPPATH . "cilogviewer";
    ...
    public function showLogs() {
        ...
        if(!empty($this->CI->input->get("del"))) {
        ...
        if(!empty($dlFile) && file_exists($this->logFolderPath . "/" . basename(base64_decode($dlFile))) ) {
            $file = $this->logFolderPath . "/" . basename(base64_decode($dlFile));
            $this->downloadFile($file);
        }

        if(!empty($this->CI->input->get(self::API_QUERY_PARAM))) {
        ...
        else if(empty($fileName) && !empty($files)) {
        ...
        $data['currentFile'] = !empty($currentFile) ? basename($currentFile) : "";
    ...
    private function processAPIRequests($command) {
        ...
        if($command === self::API_CMD_LIST) {
            ...
            if(empty($file) || empty($file)) {
                ...
                $singleLine = !empty($singleLine) && ($singleLine === true || $singleLine === "true" || $singleLine === "1") ? true : false;
...
//               ]);
    ...
    private function deleteFiles($fileName) {
        ...
        if($fileName == "all") {
            ...
            echo $this->logFolderPath . "/" . basename($fileName);exit;
    ...
    private function prepareRawFileName($fileNameInBase64) {
        ...
        //let's determine what the current log file is
        if(!empty($fileNameInBase64) && !empty($fileNameInBase64)) {
