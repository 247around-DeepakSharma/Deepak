                ...
                <table class = "table table-condensed table-bordered table-striped table-responsive">
                    ...
                    <?php
                        if (!empty($data)) {
                            ...
                            $jsonData = json_encode(array("service" => $row['service_id'], "request_type"=> $row['request_type_id'], "symptom" => $row['technical_solution']));
                        ?>
                    <tr>
                    ...
                    <form class="form-horizontal">
                       <div class="row">
                        ...
                        <div class="modal-footer">
                            <input type="hidden" id="symptom_completion_solution_id">
...
<script>
    function update_status(status, id){
        ...
        $.ajax({
            type: 'POST',
            url: '<?php echo base_url(); ?>employee/booking_request/update_symptom_completion_solution', 
    ...
    function update_spare_sympton(btn, id){
        ...
        $("#symptom_completion_solution_id").val(id);
    ...
    function update_spare_symptom(){
        if(!$("#model_request_type").val()){
           ...
           alert("Please Enter Completion Solution Symptom"); 
        }
        else{
            var data = {};
            data.request_type = $("#model_request_type").val();
            data.technical_solution = $("#model_symptom").val();
            $.ajax({
                type: 'POST',
                url: '<?php echo base_url(); ?>employee/booking_request/update_symptom_completion_solution', 
                data:{data:data, id : $("#symptom_completion_solution_id").val()},
                success: function (response) {
                    if(response){
                        alert("Completion Solution Symptom Updated Successfully");
                        location.reload();
                    }
                    else{
