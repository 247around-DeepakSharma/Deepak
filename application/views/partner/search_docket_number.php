...
<script>
    ...
    $(document).ready(function () {
        $('#get_docket_number_data').click(function () {
            ...
            if(docket_number || shipment_daterange){
                    ...
                    $.ajax({
                        method: 'POST',
                        data: {docket_number: docket_number,search_by:search_by,from_date:from_date,to_date:to_date,partner_id:'<?php echo $this->session->userdata('partner_id');?>'},
    ...
    function create_table(table_data){
        ...
        $.each(table_data, function (index,val) {
            ...
            table_body += "<td><a href='<?php echo base_url();?>partner/booking_details/"+ val['booking_id'] +"' target='_blank'>"+val['booking_id']+" </a></td>";
