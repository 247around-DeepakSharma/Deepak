<div role="tabpanel" class="tab-pane active" id="today_booking">
                                        ...
                                        <tr  style="text-align: center;"  >
                                            ...
                                            <td style="vertical-align: middle;     max-width: 200px;">
                                                ...
                                                <?php if ($row->booking_files_purchase_invoice) { ?>
                                                    <br/>
                                                    <a target='_blank' href="https://s3.amazonaws.com/<?php echo BITBUCKET_DIRECTORY; ?>/purchase-invoices/<?php echo $row->booking_files_purchase_invoice; ?>"  title = 'Purchase Invoice Verified' aria-hidden = 'true'><img src="<?php echo base_url(); ?>images/varified.png" style="width:20%"/></a>
                                                <?php } ?>
                                                <br/>
                                                <a href="javascript:void(0)" data-popover="true" style="border:0px; white-space:nowrap; overflow:hidden;text-overflow:ellipsis;max-width: 125px;" data-html="true" data-content="<?php if (isset($row->part_cancel_reason)) {
        echo implode('<br>', explode(',', $row->part_cancel_reason));
    } ?>" > <img style="width: 83%;" class="<?php echo 'spare_today' . $key; ?>" id="<?php echo 'spare_today' . $key; ?>" src="<?php echo base_url(); ?>images/loader.gif" /></a>
                                                <a href="javascript:void(0)" id="auot_delivered_<?php echo$key; ?>" class="spare_auto_delivered"> <img style="width: 83%;" class="<?php echo 'spare_delivered_today' . $key; ?>" id="<?php echo 'spare_delivered_today' . $key; ?>" src="<?php echo base_url(); ?>images/loader.gif" /></a>
                                                ...
                                                <script> $(document).ready(function () {
                                                        load_cancelled_status('<?php echo $row->booking_id; ?>', '<?php echo $key; ?>', 'today');
                                                        load_delivered_status('<?php echo $row->booking_id; ?>', '<?php echo $key; ?>', 'today');
                                                        load_spare_cost_status('<?php echo $row->booking_id; ?>', '<?php echo $key; ?>', 'today');
                                                        load_rejected_from_review('<?php echo $row->booking_id; ?>', '<?php echo $key; ?>', 'today');
                                                    });
                                        ...
                                        <?= $row->customername . "<br/>" . $row->booking_primary_contact_no; ?>
                                         ...
                                         <?php echo $row->booking_address . ", " . $row->booking_pincode; ?> 
                                            ...
                                            <td style="vertical-align: middle;"> <?php if (isset($row->age_of_booking)) {
                                                echo $row->age_of_booking . " day";
                                            } ?></td>
                                            <td data-popover="true" style="border:0px; white-space:nowrap; overflow:hidden;text-overflow:ellipsis;max-width: 125px;" data-html=true data-content="<?php if (isset($row->admin_remarks)) {
                                                echo $row->admin_remarks;
                                            } ?>">
                                            <?php if (isset($row->admin_remarks)) {
                                                echo $row->admin_remarks;
                                            } ?>
                                            </td>
                                                    <?php if ($this->session->userdata('is_update') == 1) { ?>
    ...
    if (isset($saas_module) && (!$saas_module)) {
        ?>
            ...
            <?php } else { ?>
                    ...
                    echo " disabled";
                } ?>"  title="Edit Request Type"><i class="fa fa-edit" aria-hidden="true"></i></a>
            <?php }
        } else { ?>
                                                        <a href="#" style="width: 36px;background: #795b95;border: #795b95;" class="btn btn-sm btn-primary  disabled"  title="Edit Request Type"><i class="fa fa-edit" aria-hidden="true"></i></a>
                                                <?php } ?>
                                            ...
                                            <td style="vertical-align: middle;">
                                                ...
                                                $disabled_flag = false;
                                                if ($row->nrn_approved == 0) {
                                                    if ($this->session->userdata('is_update') == 1) {
                                                        if ($is_engineer_app == '1') {
                                                            if (is_null($row->assigned_engineer_id) || (!empty($row->is_booking_close_by_app_active))) {
                                                                $disabled_flag = true;
                                                                ?>                                              
                                                    ...
                                                    //  If engineer is not assigned, SF already completed the booking, Admin completed the booking do not allow SF to complete booking                                               
                                                    if (($row->service_center_current_status == SF_BOOKING_INPROCESS_STATUS && !empty($row->service_center_closed_date) && empty($disabled_flag)) || ($row->service_center_current_status == _247AROUND_COMPLETED)) {
                                                        $disabled_flag = true;
                                                        ?>
                                                        <a href="#" class='btn btn-sm btn-success disabled' title='Complete'><i class='fa fa-thumbs-up' aria-hidden='true'></i></a> <?php
                                                    }
                                                    if (empty($disabled_flag)) {
                                                        ?>
                                                        <a href="<?php echo base_url(); ?>service_center/get_sf_edit_booking_form/<?php echo urlencode(base64_encode($row->booking_id)); ?>/<?php echo urlencode(base64_encode($redirect_url)) ?>" class='btn btn-sm btn-success' title='Complete'><i class='fa fa-thumbs-up' aria-hidden='true'></i></a> <?php
                                            }
                                        } else {
                                            ?>
                                                    <a href="<?php echo base_url(); ?>service_center/get_sf_edit_booking_form/<?php echo urlencode(base64_encode($row->booking_id)); ?>/<?php echo urlencode(base64_encode($redirect_url)) ?>" class='btn btn-sm btn-success <?php if ($this->session->userdata('is_update') == 1) { ?> <?php if ($is_engineer_app == '1') {
                                                    if (is_null($row->assigned_engineer_id) || (!empty($row->is_booking_close_by_app_active))) { ?>  disabled <?php }
                                                }
                                            } if (($row->service_center_current_status == SF_BOOKING_INPROCESS_STATUS && !empty($row->service_center_closed_date)) || ($row->service_center_current_status == _247AROUND_COMPLETED)) {
                                                echo 'disabled';
                                            } ?>' title='Complete'><i class='fa fa-thumbs-up' aria-hidden='true'></i></a> <?php }
                                        ?>
                                            </td>
                                            <td style="vertical-align: middle;">
    <?php
    if ($row->nrn_approved == 0) {
        ...
        <?php } ?>
    <?php } else { ?>
                                                    <a href="#" class='btn btn-sm btn-danger disabled' title='Cancel' <?php if (($row->service_center_current_status == SF_BOOKING_INPROCESS_STATUS && !empty($row->service_center_closed_date)) || ($row->service_center_current_status == _247AROUND_COMPLETED)) {
            echo 'disabled';
...
<?php if ($this->session->userdata('is_update') == 1) { ?>
    ...
    foreach ($bookings[2] as $key => $row) { ?>
        ...
        <?php } ?>
                                                    ...
                                                    <a href="javascript:void(0)" data-popover="true" style="border:0px; white-space:nowrap; overflow:hidden;text-overflow:ellipsis;max-width: 125px;" data-html="true" data-content="<?php if (isset($row->part_cancel_reason)) {
            echo implode('<br>', explode(',', $row->part_cancel_reason));
        } ?>"  > <img class="<?php echo 'spare_tomorrow' . $key; ?>" id="<?php echo 'spare_tomorrow' . $key; ?>" src="<?php echo base_url(); ?>images/loader.gif"  style="width:84%"/></a>
                                                    ...
                                                    <script> $(document).ready(function () {
                                                            load_cancelled_status('<?php echo $row->booking_id; ?>', '<?php echo $key; ?>', 'tomorrow');
                                                            load_delivered_status('<?php echo $row->booking_id; ?>', '<?php echo $key; ?>', 'tomorrow');
                                                            load_spare_cost_status('<?php echo $row->booking_id; ?>', '<?php echo $key; ?>', 'tomorrow');
                                                            load_rejected_from_review('<?php echo $row->booking_id; ?>', '<?php echo $key; ?>', 'tomorrow');
                                                        });
                                                ...
                                                <?php if ($row->booking_files_purchase_invoice) { ?>
                                                        <br/>
                                                        <a target='_blank' href="https://s3.amazonaws.com/<?php echo BITBUCKET_DIRECTORY; ?>/purchase-invoices/<?php echo $row->booking_files_purchase_invoice; ?>"  title = 'Purchase Invoice Verified' aria-hidden = 'true'><img src="<?php echo base_url(); ?>images/varified.png" style="width:30%"/></a>
                                                <?php } ?>
                                                </td>
                                                <td style="max-width: 100px; word-wrap:break-word;vertical-align: middle;">
        <?= $row->customername . "<br/>" . $row->booking_primary_contact_no; ?>
                                                ...
                                                <td style="max-width: 100px; word-wrap:break-word;vertical-align: middle;">
                                                <?php if (stristr($row->request_type, "Installation")) {
                                                    if ($row->amount_due > 0) { ?> <span style="font-weight:bold">Paid </span> <?php } else { ?> <span style="font-weight:bold">Free </span><?php }
                                } echo $row->request_type . " " . $row->services; ?>
                                                </td>
                                                <td style="max-width: 100px; word-wrap:break-word;vertical-align: middle;">
        <?= $row->partner_internal_status; ?>
                                                </td>
                                             ...
                                             <?php if(!empty($row->booking_date) &&  $row->create_date != '0000-00-00'){echo  date('d-M-Y h:i:s', strtotime($row->create_date))."<br/>"; }?> 
                                                ...
                                                <td data-popover="true" style="position: absolute; border:0px; white-space:nowrap; overflow:hidden;text-overflow:ellipsis;max-width: 125px;" data-html=true data-content="<?php if (isset($row->admin_remarks)) {
                                            echo $row->admin_remarks;
                                        } ?>">
                                                        <?php if (isset($row->admin_remarks)) {
                                                            echo $row->admin_remarks;
            ...
            <?php //}  ?>
                                                        ...
                                                        <?php
                                                        //foreach ($engineer_details as $value1) {
        ...
        <?php if ($row->nrn_approved == 0) { ?>
            ...
            } ?>' title='Complete'><i class='fa fa-thumbs-up' aria-hidden='true'></i></a>
        <?php } else { ?>
    ...
    <?php } ?>
            ...
            <?php } ?>
                                                        ...
                                                        <a href="javascript:void(0)" data-popover="true" style="width:10px;border:0px; white-space:nowrap; overflow:hidden;text-overflow:ellipsis;max-width: 125px;" data-html="true" data-content="<?php if (isset($row->part_cancel_reason)) {
                echo implode('<br>', explode(',', $row->part_cancel_reason));
            } ?>" > <img class="<?php echo 'spare_rescheduled' . $key; ?>" id="<?php echo 'spare_rescheduled' . $key; ?>" src="<?php echo base_url(); ?>images/loader.gif"  style="width:84%"/></a>
                                                        <a href="javascript:void(0)" > <img style="width: 83%;" class="<?php echo 'spare_delivered_rescheduled' . $key; ?>" id="<?php echo 'spare_delivered_rescheduled' . $key; ?>" src="<?php echo base_url(); ?>images/loader.gif" /></a>
                                                        <a href="javascript:void(0)" > <img style="width: 89%;" class="<?php echo 'spare_cost_given_rescheduled' . $key; ?>" id="<?php echo 'spare_cost_given_rescheduled' . $key; ?>" src="<?php echo base_url(); ?>images/loader.gif" /></a>
                                                        <script> $(document).ready(function () {
                                                                load_cancelled_status('<?php echo $row->booking_id; ?>', '<?php echo $key; ?>', 'rescheduled');
                                                                load_delivered_status('<?php echo $row->booking_id; ?>', '<?php echo $key; ?>', 'rescheduled');
                                                                load_spare_cost_status('<?php echo $row->booking_id; ?>', '<?php echo $key; ?>', 'rescheduled');
                                                            });
            ...
            <?php if ($row->nrn_approved == 0) { ?>
                        ...
                        if (is_null($row->assigned_engineer_id) || !empty($row->is_booking_close_by_app_active)) {
                            ?>  disabled <?php }
                                                    ...
                                                    <td style="vertical-align: middle;"><a href="https://s3.amazonaws.com/<?php echo BITBUCKET_DIRECTORY; ?>/jobcards-pdf/<?php echo $row->booking_jobcard_filename ?> " class='btn btn-sm btn-warning btn-sm <?php if ($this->session->userdata('is_update') == 1) { ?><?php if (is_null($row->assigned_engineer_id)) { ?>  //disabled <?php }
    ...
    foreach ($spare_parts_data as $key => $row) {
        ...
        <?php echo $row['parts_requested']; ?>
                                            ...
                                            <td>
        <?php if ($row['shipped_date'] != "0000-00-00") {
            echo $row['shipped_date'];
        } ?>
                                            </td>     
