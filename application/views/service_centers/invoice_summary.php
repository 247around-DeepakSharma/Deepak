  ...
  <table class="table table-bordered  table-hover table-striped data"  >
   <thead>
      <tr >
         ...
         <th>Paid Amount</th>
         <th>Amt Paid by Partner</th>
         <th>Paid by 247around</th>
         <th>Sent Date</th>
   ...
   <tbody>
      ...
      <tr <?php if($invoice['settle_amount'] == 1){ ?> style="background-color: #90EE90; " <?php } ?>>
         ...
         <td><?php echo $invoice['total_amount_collected']; $total_amount_collected += $invoice['total_amount_collected'];?></td>
          <td><?php echo $invoice['tds_amount']; ?></td>
         <td><?php echo $invoice['around_royalty']; $around_royalty += $invoice['around_royalty']; ?></td>
         <?php $amount_collected_paid += $invoice['amount_collected_paid']; ?>

          <td><?php if(abs($invoice['amount_collected_paid']) == $invoice['amount_paid'] ){ echo abs($invoice['amount_paid']); } else { echo abs($invoice['amount_collected_paid'] + $invoice['amount_paid']);}   ?></td>

         <?php $amount_collected_paid += $invoice['amount_collected_paid']; ?>
         <td><?php if($invoice['amount_collected_paid'] >0){ echo abs($invoice['amount_collected_paid']) - $invoice['amount_paid'];} else { echo "0.00"; }?></td>
         <td><?php if($invoice['amount_collected_paid'] <=0){ echo abs($invoice['amount_collected_paid']) + $invoice['amount_paid'] ; } else { echo "0.00"; } ?></td>

         <?php } else if($invoice['type'] == "FOC"){ ?>

         <td><?php echo (($invoice['total_service_charge'] + $invoice['service_tax'] )* basic_percentage); $sum_of_total_service_charges +=  (($invoice['total_service_charge'] + $invoice['service_tax']) * basic_percentage); ?></td>

         <td><?php echo ($invoice['total_additional_service_charge'] * basic_percentage); $sum_total_additional_service_charge += ($invoice['total_additional_service_charge'] *basic_percentage);?></td>

         <td><?php echo (($invoice['parts_cost'] + $invoice['vat'])*basic_percentage); $sum_total_parts_cost += (($invoice['parts_cost'] + $invoice['vat'])*basic_percentage ); ?></td>
         

         <td><?php echo ((($invoice['total_service_charge'] + $invoice['service_tax'] )* basic_percentage) + ($invoice['total_additional_service_charge'] * basic_percentage) + (($invoice['parts_cost'] + $invoice['vat'])*basic_percentage)); ?></td>

         <td><?php echo $invoice['tds_amount']; ?></td>

         <td>0.00</td>
         <td><?php if(abs($invoice['amount_collected_paid']) == $invoice['amount_paid'] ){ echo $invoice['amount_paid']; } else { echo abs($invoice['amount_collected_paid'] + $invoice['amount_paid']);}   ?></td>
         <?php $amount_collected_paid += $invoice['amount_collected_paid']; ?>
