...
{
$data=mysqli_query($db,"SELECT DISTINCT(brand_name) FROM appliance_brands WHERE `service_id`=46 AND `seo`=1")or die("Error");
...
{
	$data=mysqli_query($db,"SELECT Keyword FROM workbook2")or die(mysqli_error());
...
function get_url($db)
{
$data=mysqli_query($db,"SELECT * FROM workbook2")or die(mysqli_error());
...
while($row =mysqli_fetch_assoc($data))
{
	...
	array_push($urls,str_replace(" ","-", $url));
}
...
**/
function update_sitemap($url,$sitemap)
{
/*
 * SITEMAP TEMPLATE
  <url>
...
** 
**/
function insert_titles_of_non_template($db,$sitemap_file,$total_rows_inserted)
{
	echo "\nInserting urls of blogs table which are not in template form in the url table\n\n";
	$data=mysqli_query($db,"SELECT url,id FROM blogs where is_template=0")or die(mysqli_error());
	...
	$res="INSERT IGNORE INTO `url_table_tmp` (`url`, `brand`, `place`, `blog_id`, `create_date`) VALUES ";	
	while($row =mysqli_fetch_assoc($data))
	{
		$title=$row['url'];
		$id=$row['id'];
		...
		update_sitemap($title,$sitemap_file);
	}
...
**/
function make_copies_of_url($brand,$suffixes,$to_replace,$res,$sql,$url,$sitemap_file)
{
	...
	for ($j=0; $j <sizeof($suffixes) ; $j++)
	{
		...
		update_sitemap($final_url,$sitemap_file);
...
//these two staements ensure that all work is undone form url_table_tmp and status in workbook is set to "tobedone"
$delete=mysqli_query($db,"DELETE FROM url_table_tmp")or die(mysqli_error());
if($delete!=false)
	echo mysqli_affected_rows($db)." rows are deleted from url_table_tmp\n\n";
else 
	echo "Error ocurred while deleting rows from url_table_tmp\n\n";


$update_status=mysqli_query($db,"UPDATE workbook2 SET status='to be done',sitemap='to be done'")or die(mysqli_error());
if($update_status!=false)
	echo "Status of all ".mysqli_affected_rows($db)." rows is updated to undone in workbook table\n\n";
else
...
for($k=0;$k<sizeof($urls);$k++) 
{
	...
	if($is_template)
	{
		...
		if(strpos($urls[$k],"<brand>")!==false)
		{
			...
			for($i=0;$i<sizeof($brands);$i++)
			{
				...
				if(is_suffix_exists($urls[$k]))
				{
					...
					if(strpos($urls[$k],"<place>")!==false)
						$res=make_copies_of_url($brands[$i],$places,"<place>",$res,$sql,$urls[$k],$sitemap_file);
					else if(strpos(strtolower($urls[$k]),"<region>")!==false)
						$res=make_copies_of_url($brands[$i],$regions,"<region>",$res,$sql,$urls[$k],$sitemap_file);
					else if(strpos(strtolower($urls[$k]),"<area>")!==false)
						$res=make_copies_of_url($brands[$i],$areas,"<area>",$res,$sql,$urls[$k],$sitemap_file);
					else if(strpos(strtolower($urls[$k]),"<pincode>")!==false)
						$res=make_copies_of_url($brands[$i],$pincodes,"<pincode>",$res,$sql,$urls[$k],$sitemap_file);	
					else if(strpos(strtolower($urls[$k]),"<city>")!==false)
						$res=make_copies_of_url($brands[$i],$cities,"<city>",$res,$sql,$urls[$k],$sitemap_file);	
				}
				else
				{
		...
		else if(is_suffix_exists($urls[$k]))
		{
			...
			$res="INSERT IGNORE INTO `url_table_tmp` (`url`, `brand`, `place`, `blog_id`, `create_date`) VALUES ";
			if(strpos($urls[$k],"<place>")!==false)
				$res=make_copies_of_url("",$places,"<place>",$res,$sql,$urls[$k],$sitemap_file);
			else if(strpos(strtolower($urls[$k]),"<region>")!==false)
				$res=make_copies_of_url("",$regions,"<region>",$res,$sql,$urls[$k],$sitemap_file);
			else if(strpos(strtolower($urls[$k]),"<area>")!==false)
				$res=make_copies_of_url("",$areas,"<area>",$res,$sql,$urls[$k],$sitemap_file);
			else if(strpos(strtolower($urls[$k]),"<pincode>")!==false)
				$res=make_copies_of_url("",$pincodes,"<pincode>",$res,$sql,$urls[$k],$sitemap_file);	
			else if(strpos(strtolower($urls[$k]),"<city>")!==false)
				$res=make_copies_of_url("",$cities,"<city>",$res,$sql,$urls[$k],$sitemap_file);	
...
$total_rows_inserted=insert_titles_of_non_template($db,$sitemap_file,$total_rows_inserted);
